...
# read in the grids
grids_1 <- readRDS("Spatial data/study_extent_grids_1km.RDS")
...
assign_grids_to_dat <- function(file_name){
  ...
  grids_1_lookup <- localities_sf %>%
    st_intersects(grids_1) %>%
    as.data.frame() %>%
  ...
  saveRDS(grids_1_lookup, paste0("Data/grid_lookups/1km_grid_lookup_", gsub("_data.RDS", "", file_name), ".RDS"))
